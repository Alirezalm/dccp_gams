...
class SparseQCQP(RandDCCP):
    ...
    def _objective(self):
        z = zeros((self.nVars, 1))
        for i in range(self.nVars):
            z[i] = sum([self.model.x[j] * self.obj_data['hess'][j, i] for j in range(self.nVars)])
    ...
    def create_constraints(self, bound):
        ...
        z = zeros((self.nVars, 1))
        for i in range(self.nVars):
            z[i] = sum([self.model.x[j] * self.constr_data['hess'][j, i] for j in range(self.nVars)])
